...
function useFocusTrap(active = true) {
  ...
  const focusNode = (node) => {
    ...
    if (focusElement) {
      ...
      console.warn("[@ns-ui/hooks/use-focus-trap] Failed to find focusable element within provided node", node);
    }
  };
    ...
    if (node) {
      setTimeout(() => {
        if (node.getRootNode()) {
          ...
          console.warn("[@ns-ui/hooks/use-focus-trap] Ref node is not part of the dom", node);
        }
      });
      ref.current = node;
    } else {
