...
const HorizontalSection = forwardRef((_a, ref) => {
  ...
  const breakpoints = getSortedBreakpoints(width, theme).reduce((acc, [breakpoint, breakpointSize]) => {
    acc[`@media (min-width: ${em(breakpoint)})`] = {
      [`--ns_ui-${section}-width`]: rem(breakpointSize)
    };
    return acc;
  }, {});
  return /* @__PURE__ */ React.createElement(Box, __spreadValues({
    ...
    styles: () => ({
      ":root": __spreadValues({
        [`--ns_ui-${section}-width`]: (width == null ? void 0 : width.base) ? rem(width.base) : "0rem"
      }, breakpoints)
    })
  }));
});
HorizontalSection.displayName = "@ns-ui/core/HorizontalSection";
